...
import { Search, Filter, Microscope, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500">Pending Scans</p>
              <p className="mt-1 text-3xl font-semibold text-gray-900">
                {radiologyResults.filter(r => r.status === 'pending').length}
              </p>
            </div>
            <div className="p-3 rounded-full bg-warning-100">
              <Microscope className="h-6 w-6 text-warning-500" />
            </div>
          </div>
        </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-3xl font-semibold text-gray-900">
                {radiologyResults.filter(r => 
                  r.status === 'completed' && 
                  new Date(r.scan_date).toDateString() === new Date().toDateString()
                ).length}
              </p>
            </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-3xl font-semibold text-gray-900">
                {radiologyResults.filter(r => r.is_emergency).length}
              </p>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredResults.map((result) => (
                <tr key={result.id} className={result.is_emergency ? 'bg-error-50' : ''}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {result.scan_type.split('_').map(word => 
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <a href={`/radiology/${result.id}`} className="text-primary-600 hover:text-primary-900">
                      View Results
                    </a>
                  </td>
                </tr>
